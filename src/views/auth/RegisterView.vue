...
const passwordError = computed(() => {
  ...
  if (password.value.length < 6 || password.value.length > 20) return "密码长度需为 6-20 个字符";
  return "";
});
...
<template>
  <div class="register-page">
    <section class="register-art" aria-label="游戏宣传区域">
      ...
      <div class="brand-hero"><span>996</span><strong>传奇盒子</strong></div>
      <div class="game-logo"><small>996</small><b>传奇世界</b><i>热血 · 兄弟 · 荣耀</i></div>
    ...
    <section class="register-panel">
      ...
      <div class="panel-content">
        ...
        <h1>注册账号</h1>
        <p class="subtitle">996传奇引擎玩家可直接使用游戏账号登录</p>
        ...
        <form @submit.prevent="register" novalidate>
          ...
          <label class="input-wrap" :class="{ invalid: passwordError }"><span class="input-icon">♙</span><input v-model="password" :type="showPassword ? 'text' : 'password'" autocomplete="new-password" placeholder="请设置密码，长度 6-20 个字符" @blur="touched = true" /><button type="button" class="eye-button" @click="showPassword = !showPassword">{{ showPassword ? "◉" : "⌁" }}</button><span v-if="passwordError" class="field-error">{{ passwordError }}</span></label>
          <label class="input-wrap" :class="{ invalid: confirmationError }"><span class="input-icon">♙</span><input v-model="passwordConfirmation" :type="showConfirmation ? 'text' : 'password'" autocomplete="new-password" placeholder="再次输入密码，请保持一致" @blur="touched = true" /><button type="button" class="eye-button" @click="showConfirmation = !showConfirmation">{{ showConfirmation ? "◉" : "⌁" }}</button><span v-if="confirmationError" class="field-error">{{ confirmationError }}</span></label>
