...
const activeTab = ref(new URLSearchParams(window.location.hash.split("?")[1] || "").get("tab") || "overview");
...
const tabs = [
  ...
  { key: "live", label: "我要直播", highlight: true },
];
...
function openLiveStudio() { window.location.hash = "#/live/create"; }
...
<template>
  <div class="account-page">
    ...
    <nav><button v-for="tab in tabs" :key="tab.key" :class="{ active: activeTab === tab.key, highlight: tab.highlight }" @click="tab.key === 'live' ? openLiveStudio() : activeTab = tab.key">{{ tab.label }}<b v-if="tab.key === 'messages' && unreadCount">{{ unreadCount }}</b></button></nav>
    ...
    <main v-else class="panel">
      ...
      <template v-else-if="activeTab === 'live'"><div class="empty"><p>我要直播已升级为独立直播工作台。</p><button class="live-button" @click="openLiveStudio">进入直播工作台</button></div></template>
...
<style scoped>
:global(*){box-sizing:border-box}:global(body){margin:0;background:#111217;color:#f5f1e9;font-family:Inter,"PingFang SC","Microsoft YaHei",sans-serif}button{font:inherit;cursor:pointer;border:0;background:transparent;color:inherit}.account-page{max-width:1180px;min-height:100vh;margin:auto;padding:32px 42px;background:#111217}header{display:flex;align-items:center;justify-content:space-between;margin-bottom:25px}header>div{text-align:center}header small{color:#c7a248;letter-spacing:2px;font-size:10px}h1{margin:5px 0;font-size:24px}.back,.message{color:#9a9ba4}.message{position:relative;font-size:21px}.message i,nav b{margin-left:5px;border-radius:8px;background:#d85e65;color:white;padding:2px 5px;font-size:9px;font-style:normal}.profile{display:flex;align-items:center;gap:20px;padding:26px 30px;border:1px solid #343038;border-radius:10px;background:linear-gradient(105deg,#25231e,#1b1c23 65%)}.avatar{width:70px;height:70px;display:grid;place-items:center;border:2px solid #c8a43e;border-radius:50%;background:#35333a;color:#edc957;font-size:25px}.profile h2{margin:0}.profile h2 b{padding:4px 7px;border-radius:3px;background:#503c1a;color:#e7c659;font-size:10px}.profile p,small{display:block;color:#858790;font-size:11px}.profile button,footer button{color:#d7b94e}nav{display:flex;gap:25px;margin:28px 0 20px;border-bottom:1px solid #2b2d34}nav button{padding:0 0 14px;color:#85868f;font-size:12px}nav button.active{color:#e6c653;border-bottom:2px solid #d7b64f}nav button.highlight{color:#d85e65}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.stats article,.panel{padding:20px;border:1px solid #2e3038;border-radius:8px;background:#1a1b21}.stats strong{display:block;margin-top:10px;color:#f0ce64;font-size:21px}.panel{margin-top:16px}.panel h2{margin-top:0}.row{display:flex;align-items:center;gap:12px;padding:14px 0;border-bottom:1px solid #292b32}.row>span{width:36px;height:36px;display:grid;place-items:center;border-radius:8px;background:#30323b;color:#f1d376;font-size:18px}.row>div{flex:1}.row time,.row em{color:#85868e;font-size:10px;font-style:normal}.setting{padding:16px 5px;border-bottom:1px solid #292b32;color:#c4c5ca;cursor:pointer}.setting:hover{color:#e6c653}.live-button{display:block;width:100%;padding:12px;border-radius:8px;background:#d85e65;color:white;font-weight:bold;margin-bottom:20px}.live-row{display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid #292b32}.live-row label{color:#85868f;font-size:12px;width:100px}.live-input{flex:1;padding:8px;border:1px solid #2e3038;border-radius:4px;background:#1a1b21;color:#f5f1e9;font-size:12px}.empty{text-align:center;padding:35px;color:#85868e}.error{color:#df7779}footer{display:flex;justify-content:space-between;margin-top:23px;color:#6f717a;font-size:10px}footer button{color:#c87579}@media(max-width:850px){.account-page{padding:25px 18px}.profile{align-items:flex-start}.stats{grid-template-columns:repeat(2,1fr)}nav{gap:15px;overflow:auto}}@media(max-width:520px){.account-page{padding:20px 14px}.profile{padding:20px}.stats{grid-template-columns:1fr 1fr}.row time{display:none}}
.live-studio-head{display:flex;align-items:flex-start;justify-content:space-between;padding:24px;border:1px solid #3a3227;border-radius:10px;background:radial-gradient(circle at 80% 0%,#40301c,#1a1b21 55%)}.live-studio-head h2{margin:6px 0;font-size:24px}.live-studio-head p{margin:0;color:#898b94;font-size:12px}.live-status{display:flex;align-items:center;gap:7px;padding:7px 10px;border:1px solid #39343a;border-radius:20px;color:#9899a2;font-size:11px}.live-status i{width:7px;height:7px;border-radius:50%;background:#777}.live-status i.ready{background:#5bc58a;box-shadow:0 0 8px #5bc58a}.live-studio-grid{display:grid;grid-template-columns:1.2fr 1fr;gap:16px;margin-top:16px}.live-preview,.live-form,.live-addresses{padding:20px;border:1px solid #2e3038;border-radius:8px;background:#1a1b21}.preview-screen{aspect-ratio:16/9;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;border-radius:6px;color:#a7a8b0;background:radial-gradient(circle at 50% 35%,#30283a,#0e1016 70%)}.preview-mark{color:#d7b54d;font-size:30px}.preview-screen strong{color:#eee;font-size:15px}.preview-screen small{margin:0;color:#858790}.preview-meta{display:flex;justify-content:space-between;margin-top:14px;color:#858790;font-size:11px}.live-form{display:flex;flex-direction:column;gap:15px}.live-form label{display:flex;flex-direction:column;gap:7px;color:#a5a6ae;font-size:11px}.live-form input{padding:11px;border:1px solid #343640;border-radius:5px;outline:0;color:#eee;background:#121319;font-size:12px}.live-form input:focus{border-color:#a47d2b}.live-button:disabled{opacity:.5;cursor:not-allowed}.live-addresses{margin-top:16px}.address-head{display:flex;align-items:flex-start;justify-content:space-between}.address-head h3{margin:0 0 7px;font-size:14px}.address-head p{margin:0;color:#858790;font-size:11px}.address-head>span{color:#6fc494;font-size:10px}.live-addresses .live-row{padding:12px 0}.live-addresses .live-row button{padding:7px 10px;border:1px solid #5d512e;border-radius:4px;color:#d6b84f;font-size:11px}@media(max-width:700px){.live-studio-grid{grid-template-columns:1fr}.live-studio-head{padding:18px}.address-head{gap:12px;flex-direction:column}}
</style>
